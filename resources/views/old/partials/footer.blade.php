<footer class="relative py-8 px-8 border border-t-1 border-dark">
    <div class="pt-10 pb-10 flex items-center justify-between">
        <div class="col-start-3 md:col-start-2 col-end-23">
            <p class="uppercase text-xxs text-xs">© Dash, part of the works <!-- -->2024</p>
        </div>
        <div class="flex space-x-5 md:space-x-7">
            <a href="/cookies" class="uppercase text-xxs md:text-xs text-black hover:text-blue-500">Cookies</a>
            <a href="/privacy-statement" class="uppercase text-xs text-black hover:text-blue-500">Privacy Statement</a>
        </div>
    </div>
</footer>
